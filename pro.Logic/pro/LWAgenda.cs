...
namespace pro.Logic
{
    public class LWAgenda
    {
        private readonly IWAgenda _interface;

        public LWAgenda(DBPRO dbc)
        {
            _interface = new RWAgenda(dbc);
        }
        ...
        {
            List<MWAgendaExportar> list = await _interface.GetFiltroExportarAsync(model);
